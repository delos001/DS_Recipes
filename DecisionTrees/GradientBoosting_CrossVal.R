...
#Fit Boost
set.seed(1)
boost.model<-gbm(TARGET_FLAG~.,data = x.train.boost,distribution="bernoulli",n.trees=x,shrinkage = x,interaction.depth=x, n.minobsinnode = x)
summary(boost.model)
#Evaluate on Validation Set
boost.pred<- predict(boost.model, x.validation, type="response", n.trees = x)
#Confusion Matrix
boost.pred <- ifelse(boost.pred > 0.50,1,0)
xtab.boost1=table(boost.pred, x.validation$TARGET_FLAG)
confusionMatrix(xtab.boost1, positive = "1")
#AUC
library(ROCR)
library(cvAUC)
labels <- x.validation[,"TARGET_FLAG"]
AUC(predictions = boost.pred, labels = labels)

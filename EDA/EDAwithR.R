...
cf_summ_stats_tbl = function(t){
  cf_summ_stats = function(x) {
    stats = data.frame(rbind(range(x),
                             ...
                             sd(x, na.rm = TRUE),
                             var(x, na.rm = TRUE)),
                       row.names = c("Range", "Min", "Q10", "Q25", "Mean", 
                                     "Med", "Q75", "Q90", "Max", "SD", 
                                     "Var"))
      colnames(stats) = colnames(x)
    return(stats)
  }
  round(t(data.frame(apply(t, 2, cf_summ_stats))),2)
}

#-----------------------------------------------------
# BASIC EDA WITH SMIMR
skim(stAEp[,c(28:31, 34, 36, 37, 39, 45, 52)])
